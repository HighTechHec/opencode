...
export class OpenAIResponsesLanguageModel implements LanguageModelV2 {
  ...
  private async getArgs({
    ...
    if (openaiOptions?.serviceTier === "flex" && !modelConfig.supportsFlexProcessing) {
      warnings.push({
        type: "other",
        message: "serviceTier=flex is only available for o3, o4-mini, and gpt-5 models",
      })
    ...
    if (openaiOptions?.serviceTier === "priority" && !modelConfig.supportsPriorityProcessing) {
      warnings.push({
        type: "other",
        message:
          "serviceTier=priority is only available for supported models (gpt-4, gpt-5, gpt-5-mini, o3, o4-mini) and requires Enterprise access. gpt-5-nano is not supported",
      })
      // Remove from args if not supported
      delete (baseArgs as any).service_tier
    }
